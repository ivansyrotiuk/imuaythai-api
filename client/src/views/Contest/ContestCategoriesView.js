...
const renderFighters = (fighters) => {
  const mappedFighters = fighters.map((fighter, index) => <tr key={ index }>
                                                            <td>
                                                              <UserAvatar size="40" name={ fighter.firstname + " " + fighter.surname || fighter.email } src={ fighter.photo } />
                                                            </td>
  ...
  return <table className="table table-hover mb-0 hidden-sm-down">
           <thead>
             <tr className="thead-default">
               <th className="col-md-1 text-center"><i className="fa fa-user-circle-o" aria-hidden="true"></i></th>
           ...
           <div className="col-md-12">
             <div className="card card-accent-primary">
               <div className="card-header">
                 <strong>{ category.name } <div className="pull-right">Fighters: { category.fighters.length }</div></strong>
               </div>
