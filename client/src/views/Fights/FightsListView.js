...
import FighterMoveListItem from './FighterMoveListItem'
import { DragDropContext } from 'react-dnd';
import HTML5Backend from 'react-dnd-html5-backend';

export const FightsListView = (props) => {
    const header = <strong>Fights list</strong>
    const mappedFights = props.fights.map((fight, index) => <FighterMoveListItem key={ index } number={ index + 1 } fight={ fight } />);
